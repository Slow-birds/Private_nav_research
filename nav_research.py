...
class NavResearch:
    ...
    def get_data(self):
        ...
        df_nav = df[["date", "nav_unit", "nav_accumulated", "nav_adjusted", self.benchmark_code, "excess_nav"]].round(4)
        # df_return
    ...
    def get_analysis_table(self):
        ...
        basic_info_table = pd.DataFrame(
            {
                ...
                "单位净值": [
                    self.df_nav.loc[self.df_nav["date"] == self.end_date, "nav_unit"].values[0].round(4)
                ],
                "累计净值": [
                    self.df_nav.loc[self.df_nav["date"] == self.end_date, "nav_accumulated"]
                    .values[0]
                    .round(4)
                ],
                "复权净值": [
                    self.df_nav.loc[self.df_nav["date"] == self.end_date, "nav_adjusted"]
                    .values[0]
                    .round(4)
                ],
            }
        ...
        # nav ratio table
        days_diff = (np.datetime64(self.end_date) - np.datetime64(self.start_date)).astype("timedelta64[D]").astype(int)
        ## total_return
        total_return = (
            list(self.df_nav["nav_adjusted"])[-1] / list(self.df_nav["nav_adjusted"])[0] - 1
        )
        excess_total_return = (
        ...
        #year return table
        df_year = (self.df_nav.sort_values("date").groupby(self.df_nav["date"].dt.to_period("Y")).tail(1))
        first_row_df_nav = self.df_nav.iloc[0:1]
        df_year = pd.concat([first_row_df_nav, df_year], ignore_index=True)
        df_year[f"{self.fund_name}_收益"] = (df_year["nav_adjusted"] / df_year["nav_adjusted"].shift(1) - 1)
        df_year[f"{self.benchmark_name}_收益"] = (df_year[self.benchmark_code] / df_year[self.benchmark_code].shift(1) - 1)
        df_year["超额收益"] = (df_year[f"{self.fund_name}_收益"] - df_year[f"{self.benchmark_name}_收益"])
        df_year["分年度业绩"] = df_year["date"].dt.year
        df_year[f"{self.fund_name}_收益"] = df_year[f"{self.fund_name}_收益"].apply(lambda x: f"{x:.2%}")
        df_year[f"{self.benchmark_name}_收益"] = df_year[f"{self.benchmark_name}_收益"].apply(lambda x: f"{x:.2%}")
        df_year["超额收益"] = df_year["超额收益"].apply(lambda x: f"{x:.2%}")
        year_return_table = df_year[["分年度业绩", f"{self.fund_name}_收益", f"{self.benchmark_name}_收益", "超额收益"]].drop(index=0)
        # drawdown table
        # 初始化
        drawdowns = []
        start_idx = None
        ...
        if start_idx is not None:
            ...
            drawdown_start_date = self.df_drawdown["date"].iloc[start_idx]
            drawdown_end_date = self.df_drawdown["date"].iloc[
                np.argmin(self.df_drawdown["fund_drawdown"][start_idx:end_idx]) + start_idx
            ]
        ...
        table_list = [basic_info_table,nav_ratio_table,year_return_table,drawdown_table]
        self.table_list = table_list
        return self.table_list
    
    def get_html(self):
        table_html_one = self.table_list[0].to_html(index=False, classes="uniform-width")
        table_html_two = self.table_list[1].to_html(index=False, classes="uniform-width")
        table_html_three = self.table_list[2].to_html(index=False, classes="uniform-width")
        table_html_four = self.table_list[3].to_html(index=False, classes="uniform-width")
        nav_line = get_nav_lines(self.df_nav, self.fund_name, self.benchmark_name)
        drawdown_line = get_drawdown_lines(self.df_drawdown, self.fund_name, self.benchmark_name)
        html = f"""
            <html>
                <head>
                    <meta charset="UTF-8">
                ...
                <body>
                    <h1>后维单页_{self.fund_name}</h1>
                    {self.post_viewpoint_content}
                    {table_html_one}
        ...
        folder_path = rf"C:\Users\17820\Desktop\Private_nav_research\docs\{self.strategy}"
        # 构建完整文件路径
    ...
    def get_plot(self):
        ...
        # 画超额收益图
        ax1.plot(
            self.df_nav["date"], self.df_nav["nav_adjusted"] - 1, color="red", label="累计收益"
        )
        ax1.plot(
            self.df_nav["date"], self.df_nav[self.benchmark_code] - 1, color="blue", label=self.benchmark_name
        )
        ax1.fill_between(
            self.df_nav["date"], self.df_nav["excess_nav"] - 1, color="gray", label="超额收益"
        )
        ...
        ax1.grid()
        # 画动态回撤图
        ax2.plot(self.df_drawdown["date"], self.df_drawdown["fund_drawdown"], color="red", label="基金回撤")
        ax2.plot(
            self.df_drawdown["date"], self.df_drawdown["benchmark_drawdown"], color="blue", label="基准回撤"
        )
        ax2.fill_between(
            self.df_drawdown["date"], self.df_drawdown["excess_drawdown"], color="gray", label="超额回撤"
        )
